window.ExperimentalScene = (function () {
    ...
    Scene.prototype.setupScene = function () {
        ...
        // ctrl0.behaviours.push(function (drawable,timePoint) {
        //     /* recalculate lighting based on hand position */
        //     var hpos = scene.playerSpatialState.hands[0].pos;
        //     var light = {position:[hpos.x, hpos.y+1, hpos.z, 0], diffuse:[0.0, 0.0, 0.0]};
        //     scene.bindLightToShaderPosition(light, scene.shaders.ads, 4);
        // });
        ...
        /* TODO consider putting cacheable loadMesh into scene */
        
        // var showText = function (textStr, )
        
        var showText = function (textStr, basePos, baseOri, dirOffset, xOffset) {
            var glyphPromises = [];
            var scale = 1.0;
            ...
            Promise.all(glyphPromises).then(function (meshes) {
                for (var i=0; i<meshes.length; i++) {
                    ...
                    glyph = new FCShapes.MeshShape(mesh, {x:basePos.x+xOffset, y:basePos.y, z:basePos.z}, {scale:scale}, baseOri, {materialLabel:'matteplastic', groupLabel:'letters'});
                    scene.addObject(glyph);
                    xOffset += dirOffset * (meshInfo.maxX*1.2*scale);
        ...
        showText('#virtualreality', {x:0, y:0.3, z:3}, {x:270/DEG, y:180/DEG, z:0}, -1, 2.5);
        showText('@meta4vr', {x:0, y:0.3, z:-3}, {x:270/DEG, y:180/DEG, z:180/DEG}, 1, -2.0)
